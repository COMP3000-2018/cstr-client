    ...
    render() {
        ...
        if (nothingThere) {
            ...
            if (unlocked.HeartRate)
                Charts.push(<Panel header="Heart Rate" headerClass="discussion-panels"><BarSeries x={[80,160]} title= {'Heart Rate'} pick={1} data={140}/></Panel>);
            if (unlocked.OxygenSaturation)
                Charts.push(<Panel header="Oxygen Saturation" headerClass="discussion-panels"><BarSeries x={[85,100]} title= {'Oxygen saturation'} pick={0} data={100}/></Panel>);
            if (unlocked.Temperature)
                Charts.push(<Panel header="Temperature" headerClass="discussion-panels"><BarSeries x={[34,42]} title= {'Temperature'} pick={1} data={35}/></Panel>);
            if (unlocked.Weight)
                Charts.push(<Panel header="Weight" headerClass="discussion-panels"><BarSeries x={[50,100]} title= {'Weight'} pick={1}  data={75}/></Panel>);
